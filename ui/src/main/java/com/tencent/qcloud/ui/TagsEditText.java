...
import com.tencent.qcloud.ui.utils.ResourceUtils;
...
public class TagsEditText extends android.support.v7.widget.AppCompatAutoCompleteTextView {
    ...
    private List<String> convertTagSpanToList(List<TagSpan> tagSpans) {
        String text = getText().toString();
        if (TextUtils.isEmpty(text))
            return null;
        String[] str = text.split(" ");
        List<String> tags = new ArrayList<>(str.length);
        for (String s : str) {
            if (!TextUtils.isEmpty(s))
                tags.add(s.trim());
            Log.d("TAG", "convertTagSpanToList: " + s);
        }
        return tags;
    }
