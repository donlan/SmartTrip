...
import dong.lan.smarttrip.ui.im.GroupManageMessageActivity;
...
public class GroupManageConversation extends Conversation {
    ...
    public String getLastMessageSummary() {
        ...
        switch (lastMessage.getPendencyType()){
            case INVITED_BY_OTHER:
                if (isSelf){
                    return App.myApp().getResources().getString(R.string.summary_me)+
                            App.myApp().getResources().getString(R.string.summary_group_invite)+
                            to+
                            App.myApp().getResources().getString(R.string.summary_group_add);
                }else{
                    if (to.equals(UserInfo.getInstance().getId())){
                        return from+
                                App.myApp().getResources().getString(R.string.summary_group_invite)+
                                App.myApp().getResources().getString(R.string.summary_me)+
                                App.myApp().getResources().getString(R.string.summary_group_add);
                    }else{
                        return from+
                                App.myApp().getResources().getString(R.string.summary_group_invite)+
            ...
            case APPLY_BY_SELF:
                if (isSelf){
                    return App.myApp().getResources().getString(R.string.summary_me)+
                            App.myApp().getResources().getString(R.string.summary_group_apply)+
                            GroupInfo.getInstance().getGroupName(lastMessage.getGroupId());
                }else{
                    return from+ App.myApp().getResources().getString(R.string.summary_group_apply)+GroupInfo.getInstance().getGroupName(lastMessage.getGroupId());
                }
    ...
    public String getName() {
        return App.myApp().getString(R.string.conversation_system_group);
    }
