...
public abstract class Message {
    ...
    void setMessageAvatar(ChatAdapter.ViewHolder viewHolder, Context context){
        if (isSelf()) {
            Glide.with(context).load(UserManager.instance().faceUrl())
                    .placeholder(R.drawable.head_me)
                    .error(R.drawable.head_me)
                    .into(viewHolder.rightAvatar);
        } else {
            Glide.with(context).load(getMessage().getSenderProfile().getFaceUrl())
                    .error(R.drawable.head_other)
    ...
    protected void clearView(ChatAdapter.ViewHolder viewHolder){
        if(viewHolder.noticePanel.getVisibility() == View.VISIBLE){
            viewHolder.noticeTittle.setText("");
            viewHolder.noticeContent.setText("");
            viewHolder.noticePanel.setVisibility(View.GONE);
