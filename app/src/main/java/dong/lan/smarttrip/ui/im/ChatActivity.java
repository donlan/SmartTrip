...
public class ChatActivity extends FragmentActivity implements ChatView {
        ...
        if (message == null) {
            ...
            if (mMessage != null) {
                if (mMessage instanceof CustomMessage){
                    int messageType = ((CustomMessage) mMessage).getAction();
                    switch (messageType){
    ...
    public void showMessage(List<TIMMessage> messages) {
        ...
        for (int i = 0; i < messages.size(); ++i){
            ...
            if (mMessage == null || messages.get(i).status() == TIMMessageStatus.HasDeleted || !mMessage.shouldShow()) continue;
            ++newMsgNum;
    ...
    public void sending() {
        if (type == TIMConversationType.C2C){
            ...
            try {
                message = new CustomMessage("{action:0,data:\"\"}");
