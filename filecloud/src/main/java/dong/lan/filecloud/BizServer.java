...
/**
 * Created by 梁桂栋 on 2016/9/13 ： 下午11:28.
...
public class BizServer {
    ...
    private BizServer(Context context) {
        ...
        cos = new COSClient(context, appid, config, "smarttrip");
    }
    ...
    public String getOnceSign() {
        ...
        String cgi = "http://onjudge.applinzi.com/SmartTrip/signature.php?" + "bucket=" + bucket + "&service=once&expired=0&path="+getPath();
        try {
            URL url = new URL(cgi);
        ...
        Log.d(TAG, "getOnceSign: "+onceSign);
        return onceSign;
    }
    ...
    public String getSign() {
        String sign = null;
        String cgi = "http://onjudge.applinzi.com/SmartTrip/signature.php?" + "bucket=" + bucket + "&service=db&expired=36000&path="+getPath();
        try {
            URL url = new URL(cgi);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            InputStream in = conn.getInputStream();
